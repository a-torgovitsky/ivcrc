...
program ivcrc_estimate, eclass		
		...
		** Code for varying coefs option, leaving this as `rankstat' to avoid duplicating the kernel section.
		** For the VARCOEF() option to work the INTGRID() and BANDWIDTH() options must be specified by the  
		** user. The integral should include number list(s) over the (sub)support of the independent variable
		** being conditioned on, similarly the bandwidth list must make sense given this support.
		qui gen `rankstat' = `varcoef' 		
		if trim("`integral'") == "" {
